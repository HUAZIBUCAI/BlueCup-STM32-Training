...
__IO uint32_t uwTick_Setting_Set_Point = 0;//控制Usart_Proc的执行速度
__IO uint32_t Uart_Rev_Data_Time = 0;//控制串口接收数据的等待时间
__IO uint32_t uwTick_LED_Buling_Time = 0;//报警灯反转
...
//用户自定义变量区
uint8_t Interface_Num;//0x00显示界面 0x10设置上报小时 0x11设置分钟 0x12设置秒
float R37_Voltage;
uint8_t K_int=1;
uint8_t LED_Crtl=0;//0打开 1关闭 控制LED报警功能
uint8_t Clock_Comp_Disp[3]={0,0,0};//比较闹钟初值 (显示专用)
uint8_t Clock_Comp_Ctrl[3]={0,0,0};//比较闹钟初值 (控制专用)
uint8_t Setting_Crtl=0;//0亮 1灭 控制设置界面翻滚
uint8_t Ctrl_Uart_Send_Time_Data_Times=0;//控制发送次数
uint8_t rx_buf[100];//接受指令存入缓冲
uint8_t rx_buf_index=0;//控制数据buf存入数据
...
//	}
		...
		if(unKey_Down==2)//按键二 改变界面
		{
			...
			{
				Interface_Num=0x00;
				Clock_Comp_Ctrl[0]=Clock_Comp_Disp[0];//跟新显示值到控制值
		...
		if(unKey_Down==3)//按键三  0x10设置上报小时 0x11设置分钟 0x12设置秒
		{
		...
		if(unKey_Down==4)//按键四  
		{
			...
			{
				if(++Clock_Comp_Disp[0]==24)Clock_Comp_Disp[0]=0;
			
			}
			else if(Interface_Num==0x11)
			{
				if(++Clock_Comp_Disp[1]==60)Clock_Comp_Disp[1]=0;			
			}

			else if(Interface_Num==0x12)
			{
				if(++Clock_Comp_Disp[2]==60)Clock_Comp_Disp[2]=0;
			}
...
void Led_Proc(void)
{
	...
	if(LED_Crtl==1)//关闭报警功能
	{
		ucLed=0x00;
	}
	else
	{
		if(R37_Voltage>=(K_int*3.3*0.1))
		{
			if((uwTick -  uwTick_LED_Buling_Time)>200)
			{
				uwTick_LED_Buling_Time = uwTick;
				ucLed^=0x01;		
			}
...
//	LCD_DisplayStringLine(Line4, Lcd_Disp_String);	
	...
	if(Interface_Num==0x00)
	{
		sprintf((char *)Lcd_Disp_String, "    V1:%4.2f",R37_Voltage);
		LCD_DisplayStringLine(Line2, Lcd_Disp_String);	
	
		sprintf((char *)Lcd_Disp_String, "    k:%3.1f",(float)K_int*0.1);
		LCD_DisplayStringLine(Line4, Lcd_Disp_String);	
	...
	if((Interface_Num>>4)==0x01)
	{
		...
		LCD_DisplayStringLine(Line2, Lcd_Disp_String);	
		if((uwTick -  uwTick_Setting_Set_Point)>=500)
		{
			uwTick_Setting_Set_Point = uwTick;
			Setting_Crtl^=0x01;//^是异或的意思 如果原值是0 新值是1 如果原值是1 新值是0
		}
...
void Usart_Proc(void)
{
	if((uwTick -  uwTick_Usart_Set_Point)<200)	return;//减速函数
	...
	{
		if(Ctrl_Uart_Send_Time_Data_Times==0)//控制只发生一次数据 虽然通过扫描只进来了5次数
		{
			sprintf(str,"%1.2f+%2.1f+%02d-%02d-%02d\r\n",R37_Voltage,(float)(0.1*K_int),(unsigned int)H_M_S_Time.Hours,(unsigned int)H_M_S_Time.Minutes,(unsigned int)H_M_S_Time.Seconds);
	...
	//处理接收串口发过来的数据
	if((uwTick-Uart_Rev_Data_Time)>=50)//
	{
